...
do_alu: {
		...
		if (mode == MODE_INT && op_size >= OP_SIZE_4) {
			...
			if (op_size >= OP_SIZE_NATIVE) {
				g(gen_3address_alu(ctx, i_size(op_size), alu, R_SCRATCH_3, reg1, reg2, 0));
#if defined(ARCH_IA64)
				g(gen_3address_alu(ctx, i_size(op_size), ALU_XOR, R_SCRATCH_1, reg1, reg2, 0));
				g(gen_3address_alu(ctx, i_size(op_size), ALU_XOR, R_SCRATCH_2, reg2, R_SCRATCH_3, 0));
				if (alu == ALU_ADD) {
...
#endif
		g(gen_3address_alu(ctx, i_size(op_size), alu, R_SCRATCH_1, reg1, reg2, mode == MODE_INT && op_size >= op_size_flags));
...
do_multiply: {
		...
		if (mode == MODE_INT && op_size == OP_SIZE_8) {
			...
			g(gen_3address_alu(ctx, OP_SIZE_8, ALU_MUL, R_SCRATCH_1, reg1, reg2, 0));
		...
		if (mode == MODE_INT && op_size == OP_SIZE_NATIVE) {
			...
			g(gen_3address_alu(ctx, op_size, ALU_MUL, R_SCRATCH_1, reg1, reg2, 1));
		...
		if (mode == MODE_INT && op_size == OP_SIZE_NATIVE) {
			...
			g(gen_3address_alu(ctx, OP_SIZE_NATIVE, ALU_MUL, R_SCRATCH_1, reg1, reg2, 0));

			g(gen_3address_rot_imm(ctx, OP_SIZE_NATIVE, ROT_SAR, R_SCRATCH_4, R_SCRATCH_1, (8U << OP_SIZE_NATIVE) - 1, 0));
...
do_shift: {
		...
		if (mode == MODE_INT && alu == ROT_SHL) {
			if (op_size < OP_SIZE_NATIVE) {
				...
				g(gen_mov(ctx, OP_SIZE_NATIVE, R_SCRATCH_4, R_SCRATCH_1));
...
do_alu: {
		if (op_size > OP_SIZE_NATIVE) {
			...
			if (alu == ALU1_NOT) {
				...
				g(gen_3address_alu(ctx, OP_SIZE_NATIVE, ALU_XOR, R_SCRATCH_1, R_SCRATCH_1, R_SCRATCH_3, 0));
				g(gen_3address_alu(ctx, OP_SIZE_NATIVE, ALU_XOR, R_SCRATCH_2, R_SCRATCH_2, R_SCRATCH_3, 0));
...
#endif
#if defined(ARCH_X86)
		g(gen_2address_alu1(ctx, op_size, alu, R_SCRATCH_1, reg1, mode == MODE_INT));
#else
		if (mode == MODE_INT) {
			bool arch_use_flags = ARCH_HAS_FLAGS;
#if defined(ARCH_POWER)
			...
			if (op_size == OP_SIZE_NATIVE) {
				...
				if (alu == ALU1_NEG) {
					g(gen_3address_alu(ctx, i_size(op_size), ALU_AND, R_CG_SCRATCH, R_SCRATCH_2, reg1, 1));
				} else if (alu == ALU1_INC) {
					g(gen_3address_alu(ctx, i_size(op_size), ALU_ANDN, R_CG_SCRATCH, R_SCRATCH_2, reg1, 1));
				} else if (alu == ALU1_DEC) {
					g(gen_3address_alu(ctx, i_size(op_size), ALU_ANDN, R_CG_SCRATCH, reg1, R_SCRATCH_2, 1));
				}
			...
			if (!arch_use_flags && !ARCH_SUPPORTS_TRAPS && ARCH_HAS_ANDN && op_size == OP_SIZE_NATIVE) {
				...
				if (alu == ALU1_NEG) {
					g(gen_3address_alu(ctx, OP_SIZE_NATIVE, ALU_AND, R_SCRATCH_3, R_SCRATCH_2, reg1, 0));
				} else if (alu == ALU1_INC) {
					g(gen_3address_alu(ctx, OP_SIZE_NATIVE, ALU_ANDN, R_SCRATCH_3, R_SCRATCH_2, reg1, 0));
				} else if (alu == ALU1_DEC) {
					g(gen_3address_alu(ctx, OP_SIZE_NATIVE, ALU_ANDN, R_SCRATCH_3, reg1, R_SCRATCH_2, 0));
				}
...
#endif
		g(gen_2address_alu1(ctx, i_size(op_size), alu, R_SCRATCH_1, reg1, mode == MODE_INT));
#endif
		if (mode == MODE_INT) {
...
static bool attr_w gen_real_constant(struct codegen_context *ctx, const struct type *t, frame_t slot_r)
{
	...
	g(gen_memcpy_to_slot(ctx, slot_r, R_SCRATCH_3, offset));
	return true;
}
...
do_alu:
	if ((SUPPORTED_FP >> real_type) & 1) {
		...
		if (1)
#elif defined(ARCH_S390)
		if ((op_size <= OP_SIZE_8 && (size_t)slot_2 * slot_size < 4096) || ctx->registers[slot_2] >= 0)
#else
...
static bool attr_w gen_record_load(struct codegen_context *ctx, frame_t slot_1, frame_t slot_r, frame_t rec_slot, frame_t flags)
{
	...
	if (TYPE_IS_FLAT(entry_type)) {
		...
		g(gen_memcpy_to_slot(ctx, slot_r, R_SCRATCH_2, (size_t)rec_slot * slot_size + data_record_offset));
		flag_set(ctx, slot_r, false);
		return true;
	}
...
#endif
#if defined(ARCH_S390)
	if (t->size == 1 && s390_inline_address(offset_src) && cpu_test_feature(CPU_FEATURE_extended_imm)) {
		short reg = ctx->registers[slot_r];
...
static bool attr_w gen_array_append_one_flat(struct codegen_context *ctx, frame_t slot_1, frame_t slot_2, frame_t slot_r, frame_t flags)
{
	...
	g(gen_scaled_array_store(ctx, R_SAVED_1, data_array_offset, slot_2));
...
static bool attr_w gen_io(struct codegen_context *ctx, frame_t code, frame_t slot_1, frame_t slot_2, frame_t slot_3)
{
	...
	for (i = 0; i < slot_1; i++) {
		...
		if (ctx->registers[output_slot] >= 0)
