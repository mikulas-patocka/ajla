...
do_bt: {
		...
		if ((alu == BTX_BT || slot_1 == slot_r) && ctx->registers[slot_1] < 0) {
			...
			g(gen_frame_get(ctx, n_op_size, garbage, slot_2, 0, R_SCRATCH_1, &reg2));
			if (mode == MODE_INT) {
				int64_t imm = (8U << op_size) - 1;
				g(gen_cmp_test_imm_jmp(ctx, INSN_CMP, n_op_size, reg2, imm, alu == BTX_BT ? COND_A : COND_AE, label_ovf));
				if (unlikely(op_size > OP_SIZE_NATIVE)) {
			...
			g(gen_address(ctx, R_FRAME, (size_t)slot_1 * slot_size, IMM_PURPOSE_STR_OFFSET, maximum(n_op_size, OP_SIZE_2)));
			if (alu == BTX_BT) {
		...
		if (mode == MODE_INT) {
			...
			if (op_size == OP_SIZE_NATIVE) {
				...
				if (alu == ALU1_NEG) {
					...
					g(gen_3address_alu(ctx, i_size(op_size), ALU_ANDN, R_CG_SCRATCH, target, reg1, 1));
				} else if (alu == ALU1_DEC) {
					g(gen_3address_alu(ctx, i_size(op_size), ALU_ANDN, R_CG_SCRATCH, reg1, target, 1));
				}
			...
			if (!arch_use_flags && !ARCH_SUPPORTS_TRAPS && ARCH_HAS_ANDN && op_size == OP_SIZE_NATIVE) {
				...
				if (alu == ALU1_NEG) {
					...
					g(gen_3address_alu(ctx, OP_SIZE_NATIVE, ALU_ANDN, R_SCRATCH_3, target, reg1, 0));
				} else if (alu == ALU1_DEC) {
					g(gen_3address_alu(ctx, OP_SIZE_NATIVE, ALU_ANDN, R_SCRATCH_3, reg1, target, 0));
				}
...
do_cvt_to_int:
		...
		if (ctx->registers[slot_r] >= 0)
			g(unspill(ctx, slot_r));
		g(gen_frame_load(ctx, OP_SIZE_INT, garbage, slot_r, 0, target));
