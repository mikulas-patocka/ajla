		...
		switch (instr) {
			...
			case P_Array_Load:
				...
				gen_am_two(am, tr->slot,
					...
					(flags & Flag_Escape_On_Slice ? OPCODE_ARRAY_ESCAPE_ON_SLICE : 0));
				if (flags & Flag_Borrow)
					ctx->local_variables_flags[tr->slot].may_be_borrowed = true;
				break;
			case P_Array_Len:
...
static pointer_t pcode_build_function_core(frame_s *fp, const code_t *ip, const pcode_t *pcode, size_t size, const struct module_designator *md, const struct function_designator *fd, void **ret_ex, ajla_error_t *err)
{
	...
	while (ctx->real_len & (CODE_ALIGNMENT - 1)) {
		if (unlikely(!array_add_mayfail(uint8_t, &ctx->real_data, &ctx->real_len, 0, NULL, ctx->err)))
			goto exception;
	}
...
skip_codegen:
	...
	fn = data_align(function, real_start(ctx) + (!is_saved ? ctx->real_len : sfd->real_size), CODE_ALIGNMENT, ctx->err);
	if (unlikely(!fn))
		goto exception;
